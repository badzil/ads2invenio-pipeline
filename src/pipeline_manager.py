...
class pipelineManager(object):
    ...
    def retrieve_bibcodes_to_extract(self):
        ...
        if status_last_extraction == 'OK' or status_last_extraction == 'NOTHING FOUND' or status_last_extraction == 'NOT VALID DIRECTORY CONTENT':
            if settings.DEBUG:
                sys.stdout.write("Last extraction was fine: proceeding with a new one \n")
    ...
    def check_last_extraction(self):
        ...
        if len(directories) == 0:
            if settings.DEBUG:
                sys.stdout.write("Checked last extraction: status returned NOTHING FOUND \n")
            ...
            for name in settings.BASE_FILES:
                if settings.BASE_FILES[name] not in elements_from_last_extraction:
                    if settings.DEBUG:
                        sys.stdout.write("Checked last extraction: status returned NOT VALID DIRECTORY CONTENT \n")
            ...
            if settings.DEBUG:
                sys.stdout.write("Checking if the list of bibcodes actually extracted is equal to the one I had to extract \n")
            ...
            if len(bibcodes_still_pending) == 0:
                if settings.DEBUG:
                    sys.stdout.write("All the bibcodes from the last extraction have been processed \n")
                else:
                    pass
            else:
                if settings.DEBUG:
                    sys.stdout.write("Checked last extraction: status returned LATEST NOT ENDED CORRECTLY \n")
                return 'LATEST NOT ENDED CORRECTLY'
        
        #if everything is Ok I return it
        if settings.DEBUG:
            sys.stdout.write("Checked last extraction: status returned OK \n")
        return 'OK'
